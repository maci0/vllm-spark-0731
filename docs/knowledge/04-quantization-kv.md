...
Independent specification of the same envelope: FlashMLA's README (["The `k_cache` formats"](https://github.com/deepseek-ai/FlashMLA/blob/main/README.md)) states the layout from the model side, detected from the last dimension of `k_cache`: V4 is 584 B per token, being a 448 B `float8_e4m3` NoPE data row followed by 128 B of 64 `bfloat16` RoPE values, with a separate 8 B scale row whose first 7 bytes are `float8_e8m0` scales covering 64 NoPE values each and an 8th padding byte; V4.1 is 528 B and its FP4 variant 288 B, with a page block holding all data rows and then all scale rows. That matches this repo's page geometry and names the V4.1 variants the `v0.29.1rc0` pin now supports (#56214). FlashMLA's kernels themselves are SM90/SM100 only (the V4.1 sparse decode and the fused norm-rope-attn-rope-cast kernel are SM100-only), so none of them is usable on the GB10's SM121.
