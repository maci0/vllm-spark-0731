...
|---|---|---|---|
| 한 대에서 빠른 코드·UI·JSON worker | Qwen3.8-27B | NVFP4 또는 FP8 + SGLang/vLLM speculative 경로 | 03-4 → 05-1 |
| 긴 문맥과 복잡한 코드·문서 supervisor | DeepSeek V4 Flash 0731 | 한 대 EXL3 recipe 또는 두 대 FP8·TP=2 recipe | 06-5 → 06-1 |
| agentic coding과 다중 tool 실험 | MiniMax M2.7/M3 | 두 대 이상에서 공개 recipe를 조건 그대로 재현 | 06-7 → 07-1 |
| 영상·음성 생성 | MiniMax-H3 | 한 대 GB10 + ComfyUI recipe | 06-8 |
| Sol max와 공정하게 비교 | 작업별 local 후보 | 같은 task·harness·tool 조건을 고정 | 10-4 |
...
|---|---|---|---|
| 06-5 DeepSeek | 대형 언어·에이전트 모델 | 긴 문맥, supervisor, 코드 | prefill·decode·aggregate tok/s |
| 06-6 Qwen | dense 언어·멀티모달 모델 | 코드, UI, JSON, worker | 단일·aggregate tok/s와 tool 성공률 |
| 06-7 MiniMax | sparse MoE 언어·에이전트 모델 | tool-heavy coding, 장기 작업 | recipe별 decode·prefill |
| 06-8 MiniMax-H3 | 영상·음성 생성 모델 | ComfyUI 영상과 오디오 | step/clip 시간과 출력 품질 |
...
1. 03-1에서 OS·driver·전력·디스크 상태를 확인한다.
2. 03-4에서 한 모델의 smoke test를 통과시킨다.
3. 04-1과 06-1에서 runtime·quant·speculative 경로를 고정한다.
4. 05-1에서 prefill·decode·end-to-end·aggregate를 분리해 측정한다.
5. tool call과 실제 agent loop가 필요하면 08-1으로 이동한다.
6. 노드를 늘릴 때는 07-1과 07-4을 먼저 읽고, 09-1에서 복구 절차를 준비한다.

## 서브챕터 안내

- 06-5. DeepSeek V4 Flash 0731: 단일 Spark EXL3와 두 대 FP8·TP=2의 차이, C1·tool loop 상태
- 06-6. Qwen3.8-27B: 단일 worker, parser·speculative decoding, Qwen3.5 architecture 표시 해석
- 06-7. MiniMax M2.7과 M3: 2·3·4대 recipe와 라이선스·custom kernel 조건
- 06-8. MiniMax-H3: 단일 Spark ComfyUI 영상·음성 recipe와 `sm_121` 커널 주의점

상세 사례의 출처와 재현 큐는 06-9 DeepSeek 사례, 06-12 Qwen 사례, 부록 B 리서치 로그에서 확인한다.
