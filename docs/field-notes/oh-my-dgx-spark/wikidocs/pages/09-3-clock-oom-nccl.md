...
이 페이지는 09-1. 발열·OOM·NCCL·펌웨어 장애의 상세 내용입니다.
...
`nvidia-smi -lgc`로 의도적으로 건 상한과 고장성 저클럭은 구분한다. 부하 중 utilization과 P-state가 높아도 `clocks.sm`과 `power.draw`가 비정상적으로 낮으면 정상 성능으로 기록하지 않는다. 먼저 같은 workload에서 clock·power·temperature·실제 tok/s를 저장하고, [ASUS GX10 노드 세팅 현장 자료](https://github.com/recrack/oh-my-dgx-spark/blob/main/docs/dgx-spark-node-setup-research-2026-08.md)의 수치는 해당 장비의 community measurement로만 비교한다.
