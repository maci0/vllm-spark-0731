...
1. 목적에 맞는 모델과 노드 수를 [모델·노드 수 리서치](../docs/dgx-spark-cluster-model-research-2026-08.md)에서 고른다.
2. 실행 전에 [첫 부팅에서 실패하지 않는 방법](03-1-first-boot-safe-environment.md)에서 inventory와 메모리 headroom을 기록한다.
3. 단일 노드는 [첫 모델을 올리고 “된다”를 증명하는 방법](03-4-single-spark-first-model.md), 다중 노드는 [두 대 연결](07-1-two-spark-cluster.md)과 [다중 Spark 확장](07-4-multi-spark-scaling.md)을 따른다.
4. 결과는 [벤치마크 설계](05-1-benchmark-design.md)의 schema에 맞춰 저장한다.
5. 장애가 나면 [장애 대응](09-1-operations-failure-recovery.md)과 아래 복구 명령 색인을 먼저 확인한다.
...
| `OBLITERATUS/Qwen3.8-27B-OBLITERATED` BF16 | vLLM TP=1 | DP/서비스 분리 | 한국어, 코드, JSON, thinking smoke test | [직접 테스트 노트](../docs/model-research-qwen38-obliterated.md), locally reproduced 기능 테스트 |
| Qwen3.8-27B community builds | 1대 NVFP4/FP8/4-bit | TP=2, DP, 2×2 pool | serving, speculative decoding, coding agent | [06-12](06-12-qwen38-community-builds.md), [원문 리서치](../docs/qwen38-community-builds-2026-08.md) |
...
| DeepSeek V4 Flash 0731 community builds | 1대 EXL3 또는 2대 TP=2 | vision shim·agent·H3 co-tenancy | 사람들이 만든 실제 응용·benchmark | [06-9 커뮤니티 제작물](06-9-deepseek-community-builds.md), [원문 리서치](../docs/deepseek-v4-flash-0731-community-builds-2026-08.md) |
...
Qwen3.8-27B를 UI·디자인 worker로 두고 DeepSeek를 supervisor로 두는 구성이 필요하면 [로컬 에이전트 운영](08-1-local-agent-operations.md)과 [2×2 구성](07-1-two-spark-cluster.md#85a-2spark-pool-두-개를-함께-운영하는-구성)을 함께 본다. 두 모델을 각각 TP=2로 실행하는 경우 전체 장비는 네 대다.

Qwen3.8의 단일 Spark recipe와 실제 활용 사례는 [06-12: Qwen3.8-27B로 사람들이 만든 것](06-12-qwen38-community-builds.md)과 [상세 리서치](../docs/qwen38-community-builds-2026-08.md)에서 원본, 최적화 recipe, OBLITERATED 파생 모델을 나눠 기록한다.

DGX Spark·GB10의 Founders Edition과 Acer·ASUS·Dell·GIGABYTE·HP·Lenovo·MSI 비교는 [01-2](01-2-gb10-vendor-comparison.md)와 [벤더 비교 리서치](../docs/dgx-spark-vendor-comparison-2026-08.md)에서, DeepSeek·Qwen·MiniMax의 모델 선택은 [06-4](06-4-model-selection.md)와 [모델 선택 리서치](../docs/dgx-spark-model-selection-research-2026-08.md)에서 관리한다.

## 13.4 노드 수와 topology 색인

| 노드 수 | 기본 topology | 스위치 | 먼저 확인할 문서 |
|---:|---|---|---|
| 1 | TP=1 | 불필요 | [첫 모델을 올리고 “된다”를 증명하는 방법](03-4-single-spark-first-model.md) |
| 2 | direct QSFP/RoCE | 불필요 | [NVIDIA Connect Two Sparks](https://github.com/NVIDIA/dgx-spark-playbooks/tree/main/nvidia/connect-two-sparks), [2대 연결](07-1-two-spark-cluster.md) |
| 3 | QSFP ring 또는 DP=3 | 공식 ring은 불필요 | [NVIDIA Connect Three Sparks](https://github.com/NVIDIA/dgx-spark-playbooks/tree/main/nvidia/connect-three-sparks), [3대 확장](07-4-multi-spark-scaling.md) |
...
| 6~8 | switch fabric, TP/PP/DP 조합 | 사실상 필요 | [다중 Spark 리서치](../docs/dgx-spark-nvidia-forum-research-2026-08.md) |
| 2×2 | pair별 direct 또는 공통 switch | 구성에 따라 다름 | [2×2 역할 분리](07-1-two-spark-cluster.md#85a-2spark-pool-두-개를-함께-운영하는-구성) |
...
| 목적 | 대표 명령 | 기준 위치 |
|---|---|---|
| 장치·driver 확인 | `nvidia-smi` | [첫 부팅](03-1-first-boot-safe-environment.md), [장애 대응](09-1-operations-failure-recovery.md) |
| 메모리·디스크 확인 | `free -h`, `df -h` | [첫 부팅](03-1-first-boot-safe-environment.md) |
| 노드 inventory | `hostnamectl`, `uname -a`, `ip -br addr` | [두 대 연결](07-1-two-spark-cluster.md) |
| RDMA device 확인 | `ibdev2netdev`, `rdma link`, `ibstat` | [두 대 연결](07-1-two-spark-cluster.md), [Mac·RDMA 리서치](../docs/dgx-spark-mac-rdma-switch-research-2026-08.md) |
| 물리 링크 확인 | `ethtool <interface>` | [두 대 연결](07-1-two-spark-cluster.md), [스위치 리서치](../docs/dgx-spark-mac-rdma-switch-research-2026-08.md) |
| endpoint health | `curl -sS http://127.0.0.1:PORT/v1/models` | [벤치마크 설계](05-1-benchmark-design.md) |
| 단일 BF16 server | `vllm serve ...` | [첫 모델을 올리고 “된다”를 증명하는 방법](03-4-single-spark-first-model.md), [루트 README](../README.md) |
| Qwen3.8 canonical server | `scripts/run-qwen38-vllm.sh` | [루트 README](../README.md), [실측 JSON](../docs/results-qwen38-vllm-parser-2026-08-21.json) |
| 기능 smoke test | `python3 tests/qwen38_smoke.py ...` | [루트 README](../README.md) |
| tool call smoke test | `python3 tests/tool_call_smoke.py --strict ...` | [루트 README](../README.md), [로컬 에이전트](08-1-local-agent-operations.md) |
| 반복 측정 | `python3 tests/repeat_benchmark.py --trials 5 ...` | [벤치마크 설계](05-1-benchmark-design.md), [루트 README](../README.md) |
| tool parser | `--enable-auto-tool-choice`, `--tool-call-parser ...` | [엔진 선택](04-1-engine-selection.md), [로컬 에이전트](08-1-local-agent-operations.md) |
| 저클럭 관찰 | `nvidia-smi --query-gpu=... -l 1` | [장애 대응](09-1-operations-failure-recovery.md#114a-저클럭저전력-상태는-정상처럼-보일-수-있다) |
| NCCL 경로 | `NCCL_DEBUG=INFO`, `NCCL_DEBUG_SUBSYS=...` | [2대 연결](07-1-two-spark-cluster.md), [포럼 리서치](../docs/dgx-spark-nvidia-forum-research-2026-08.md) |
| UCX recipe workaround | `UCX_MEM_MMAP_HOOK_MODE=none` | [2대 연결](07-1-two-spark-cluster.md#87-긴-부하에서의-ucx메모리-주의) |

단일 Spark의 canonical server 명령은 [루트 README의 실행 섹션](../README.md#실행)에 두고, 책의 다른 장에서는 같은 명령을 복사해 서로 다른 버전으로 만들지 않는다.
...
전체 참고문헌은 [책 집필용 참고문헌](../docs/dgx-spark-book-references-2026-08.md), 포럼 자료는 [NVIDIA 포럼 리서치](../docs/dgx-spark-nvidia-forum-research-2026-08.md)에서 관리한다.
