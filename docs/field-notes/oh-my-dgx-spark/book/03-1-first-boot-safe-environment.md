...
현재 저장소의 Qwen3.8 테스트는 [tests/qwen38_smoke.py](../tests/qwen38_smoke.py)로 models endpoint, 한국어·코드·JSON·멀티턴·thinking·긴 문맥·4-way 요청을 확인한다. 이 스크립트는 특정 모델의 품질 벤치가 아니라 서버 기능 smoke test다.
...
[ASUS Ascent GX10 노드 세팅 자료](../docs/dgx-spark-node-setup-research-2026-08.md)는 서버 모드 전환, 플랫폼 패키지 보호, `MemAvailable` 측정, USB-C PD·ConnectX-7 문제를 실제 장비에서 다룬다. 다만 ASUS GX10의 공장 이미지와 NVIDIA DGX Spark의 이미지가 같다고 가정하지 않는다.
